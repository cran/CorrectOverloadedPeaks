...
#'
#'@references \url{http://dx.doi.org/10.1021/acs.analchem.6b02515}
#'
...
#'@param region From an initial QC-Plot file you may reprocess a specific overloaded region. Dont forget to specify the ds parameter explicitly.
...
function(data=NULL, method=c("Isoratio","Gauss","EMG")[1], detection_limit=1, ds=NULL, silent=TRUE, testing=FALSE, attotwm=FALSE, region=NULL, peak=NULL) {
  ...
  DetermineDetectorSaturation <- function(ds, pks, inf) {
    if (is.null(ds)) {
      ds <- max(sapply(pks, function(x) {max(x[,2])}))
      c1 <- sum(sapply(pks, function(x) {sum(x[,2]==ds)}))>=3
      c2 <- sum(table(unlist(sapply(pks, function(x) {x[x[,2]>(ds/2),2]})))>=sum(sapply(pks, function(x) {sum(x[,2]==ds)}))/2)==1
      c3 <- all(sapply(pks, function(x) {max(x[,2])==ds}) == (inf[,"basePeakIntensity"]==ds))
      overloaded_peaks_present <- c1 && c2 && c3
    } else {
      overloaded_peaks_present <- any(sapply(pks, function(x) {any(x[,2]>=ds)}))
    }
  ...
  GetOverloadedRegions <- function(inf, pks, lim, allowed_scans_missing, mz_gap_allowed) {
    ...
    mzaff_all <- lapply(1:length(tmp), function(i) { sapply(tmp[[i]][,"seqNum"], function(j) { pks[[j]][which(pks[[j]][,2]>=lim),1] }) })
    mzovld <- lapply(mzaff_all, function(x) { sapply(split(unlist(x), GroupByGaps(unlist(x), mz_gap_allowed)), median) })
  ...
  ExtractRawData <- function(mz, scans, pks, lim, mz_gap_allowed) {
    ...
    idx_iso <- 1
    cor_df <- data.frame(inf[scans,c("seqNum","retentionTime")],
                         t(sapply(scans, function(j) { pks[[j]][which.min(abs(mz-pks[[j]][,1])),,drop=F] })),
                         t(sapply(scans, function(j) { pks[[j]][which.min(abs(mz+idx_iso*1.0033-pks[[j]][,1])),,drop=F] })))
    colnames(cor_df) <- c("Scan","RT","mz0","int0","mz1","int1")
    check_iso_int <- any(cor_df[,"int1"]>=detection_limit*ds)
    while (check_iso_int) {
      idx_iso <- idx_iso+1
      cor_df <- cbind(cor_df, data.frame(t(sapply(scans, function(j) { pks[[j]][which.min(abs(mz+idx_iso*1.0033-pks[[j]][,1])),,drop=F] }))))
    ...
    if ((idx[1]+corr_scan_offset[1])>=1) { idx[1] <- idx[1]+corr_scan_offset[1] } else { idx[1] <- 1 }
    if ((idx[2]+corr_scan_offset[2])<=nrow(cor_df)) { idx[2] <- idx[2]+corr_scan_offset[2] } else { idx[2] <- nrow(cor_df) }
    idx <- seq(idx[1],idx[2])
    cor_df <- cor_df[idx,]
  ...
  if (class(data)=="mzXML") {
    pks <- lapply(data[["scan"]], function(x) {as.matrix(data.frame("mz"=as.numeric(x$mass),"intensity"=as.numeric(x$peaks)))})
    tmp.rt <- sapply(data[["scan"]], function(x) {x <- strsplit(x$scanAttr," ")[[1]]; x[grep("retentionTime",x)]})
    tmp.rt <- as.numeric(gsub("S\"","",gsub("retentionTime=\"PT","",tmp.rt)))
    inf <- data.frame("seqNum"=1:length(pks), "retentionTime"=tmp.rt, "basePeakMZ"=sapply(pks,function(x){x[which.max(x[,2])[1],1]}), "basePeakIntensity"=sapply(pks,function(x){max(x[,2])}))
  }
  ...
  if (!attr(ds, "opp")) {
    ...
    if (testing) cor_df_all <- lapply(1:length(mzovld), function(i) {vector("list", length=length(mzovld[[i]]))})
    ...
    for (i in 1:length(mzovld)) {
      ...
      for (k in 1:length(mzovld[[i]])) {
        ...
        if (!silent & FALSE) {
          ...
          if (!all(apply(cor_df[,c("mz0","mz1","mz2")], 2, function(x) { diff(range(x)) < mz_dev*median(x)/10^6 }))) { warning(paste(basename(file_in), "region", i, "m/z", k, "Strong mass diff")) }
          # test if M1 and M2 are overloaded
          if (all(apply(cor_df[,c("int1","int2")], 2, function(x) { max(x) > ds_cutoff[2]*ds }))) { warning(paste(basename(file_in), "region", i, "m/z", k, "Both isotopes approach detector saturation")) }
          # test if M1 and M2 are flatened
          if (all(apply(cor_df[,c("int1","int2")], 2, is.FlatTopPeak))) { warning(paste(basename(file_in), "region", i, "m/z", k, "Both isotopes are flat-toped (while below detector saturation)")) }
        }
        
        # get the scans which need to be corrected, this is usually determined by 'ds_cutoff' and 'detection_limit'
        idx <- range(which(cor_df[,"int0"]>=(ds_cutoff[2]*detection_limit*ds))); idx <- seq(idx[1],idx[2])
        ...
        if (correct_ion_suppression) {
          ...
          if (test_ionsupp){
            ...
            cutoff_modifier <- 0.4
            idx <- range(which(cor_df[,"int0"]>=(cutoff_modifier*ds_cutoff[2]*detection_limit*ds))); idx <- seq(idx[1],idx[2])
          }
        } else {
          test_ionsupp <- FALSE
        }
        ...
        if (err_code>0) {
          ...
          if (attotwm) {
            ...
            for (attotwm_i in 1:12) {
              attotwm_r <- c(attotwm_r, max(FitGaussPeak(x=cor_df[,"RT"], y=cor_df[,"int0"], scale_range=gauss_scale, steps=gauss_steps, account_for_baseline_offset=account_for_baseline_offset, silent=silent, xlab="RT", ylab="Int",
            ...
            graphics::par(cex=1)
            gauss_fit <- FitGaussPeak(x=cor_df[,"RT"], y=cor_df[,"int0"], scale_range=gauss_scale, steps=gauss_steps, account_for_baseline_offset=account_for_baseline_offset, silent=silent, xlab="RT", ylab="Int",
    ...
    if (class(data)=="xcmsRaw") {
      ...
      data@env$intensity <- unlist(sapply(pks,function(x){x[,2]}))
    }
    if (class(data)=="mzXML") {
